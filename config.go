...
func loadConfig() *ConfigST {
	...
	if err == nil {
		...
		for i, v := range tmp.Streams {
			...
			v.VideoPacketQueue = make(chan []byte, 120)
			v.ImgQueue = make(chan image.Image, 60)
			tmp.Streams[i] = v
		}
	} else {
		log.Println("read config files wrong:", err, "now generate a default config")
